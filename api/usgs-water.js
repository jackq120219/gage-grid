...
module.exports=async function handler(req,res){
  ...
  try{
    const r=await fetch(url,{headers:{'User-Agent':'GageGrid/1.0 (public-evidence prototype)','Accept':'application/json'},signal:AbortSignal.timeout(6500)});
    if(!r.ok)throw new Error(`USGS ${r.status}`);
    const data=await r.json(),series=data?.value?.timeSeries||[];
    const candidates=series.map(s=>{const geo=s?.sourceInfo?.geoLocation?.geogLocation||{},lat=Number(geo.latitude),lng=Number(geo.longitude),values=s?.values?.[0]?.value||[],latest=[...values].reverse().find(v=>Number.isFinite(Number(v?.value))),value=Number(latest?.value);if(!Number.isFinite(lat)||!Number.isFinite(lng)||!Number.isFinite(value))return null;return{station:s?.sourceInfo?.siteName||s?.sourceInfo?.siteCode?.[0]?.value||'USGS gauge',siteCode:s?.sourceInfo?.siteCode?.[0]?.value||node.gauge,lat,lng,flowCfs:value,observedAt:latest?.dateTime||null,qualifiers:latest?.qualifiers||[],distanceMiles:miles(node.lat,node.lng,lat,lng)}}).filter(Boolean).sort((a,b)=>a.distanceMiles-b.distanceMiles);
    const nearest=candidates[0]||null;
    return res.status(200).json({ok:!!nearest,site:{id,name:node.name,lat:node.lat,lng:node.lng,coordinateBasis:'pilot node / city-area approximation, not parcel geocode'},source:{authority:'U.S. Geological Survey',dataset:'NWIS instantaneous values',parameter:'Discharge, parameter 00060',url:'https://waterdata.usgs.gov/nwis',monitoringLocation:node.gauge,selectionBasis:'preselected nearby continuous monitoring location; proximity is context, not service territory'},gaugesFound:candidates.length,nearest:nearest?{...nearest,distanceMiles:Number(nearest.distanceMiles.toFixed(1))}:null,error:nearest?null:'Selected USGS monitoring location did not return a current numeric discharge observation.',retrievedAt:new Date().toISOString(),interpretation:'Observed surface-water hydrology is public context only. It does not establish municipal water capacity, allocation, pressure, treatment headroom or serviceability.'});
  }catch(error){return res.status(200).json({ok:false,site:{id,name:node.name},source:{authority:'U.S. Geological Survey',dataset:'NWIS instantaneous values',monitoringLocation:node.gauge},error:'Live USGS water context is temporarily unavailable.',detail:String(error?.message||error),retrievedAt:new Date().toISOString()})}
};
